...
WhiteBalance::WhiteBalance () : FoldableToolPanel(this, TOOL_NAME, M("TP_WBALANCE_LABEL"), true, true), wbp(nullptr), wblistener(nullptr)
{
    ...
    itcwb_del = Gtk::manage (new Gtk::CheckButton (M("TP_WBALANCE_ITCWB_DEL")));
...
void WhiteBalance::itcwb_del_toggled ()
{
    if (batchMode) {
        if (itcwb_del->get_inconsistent()) {
            itcwb_del->set_inconsistent (false);
            itcwb_delconn.block (true);
            itcwb_del->set_active (false);
            itcwb_delconn.block (false);
        } else if (lastitcwb_del) {
            itcwb_del->set_inconsistent (true);
        }

        lastitcwb_del = itcwb_del->get_active ();
    }
    if (listener && getEnabled()) {
        if (itcwb_del->get_active ()) {
            listener->panelChanged (EvWBitcwbdel, M("GENERAL_ENABLED"));
        } else {
            listener->panelChanged (EvWBitcwbdel, M("GENERAL_DISABLED"));
        }
    }
}
